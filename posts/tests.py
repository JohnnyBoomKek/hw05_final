...
class ProfileTest(TestCase):
    ...
    def test_new_post_exists(self):
        ...
        post = Post.objects.create(
            text="This will be a new post. the only post known to mankind", author=self.user)
    ...
    def test_authed_user_can_edit(self):
        self.client.force_login(self.user)
        post = Post.objects.create(
            text='This is an unedited version of the post', author=self.user)
        self.client.post(f"/{self.user.username}/{post.id}/edit/",
                         {'text': 'This is an edited version of the post'}, follow=True)
    ...
    def test_authed_user_can_sub_unsub(self):
        self.client.force_login(self.user)
        post = Post.objects.create(
            text='If ive done everything right you should see this text', author=self.user2)
        self.client.get(f'/{self.user2.username}/follow',
                        follow=True)  # могу фолловиться
        response = self.client.get('/follow/')
        # проверяю что есть пост у авторов подписок
        self.assertContains(response, post.text)
        self.client.get(f'/{self.user2.username}/unfollow',
                        follow=True)  # могу анфоловиться
        # проверяю что нету если не подписан
        response_new = self.client.get('/follow/')
        self.assertNotContains(response_new, post.text)

    def test_unauthed_user_cant_comment(self):
        post = Post.objects.create(
            text='If ive done everything right you should see this text', author=self.user2)
        response = self.client.post(f'/{self.user2.username}/{post.id}/comment/', {
                                    'text': "test comment", 'author': self.user}, follow=True)
